...
- M1 为 370 直流减速电机（尾段推进），M2 为 N20 直流减速电机（驱动唧筒活塞实现沉浮）；两路各自推/拉/停（推进侧对应前进/停止/后退）。
- 每条 CMD 是两路电机和舵机的完整目标。单路停止只停该路；底部“立即安全停止”显式同时停止两路。
- 舵机预设及滑杆为 -15° / 0° / +15°，1° 步进；发送协议模型拒绝越界值。
- 旧资料曾称配套固件两路均为固定 100% 占空比。现固件为 **2 s 慢周期 + 95%/60% 双挡位**，APK 侧发送 `step_speed=60` 切换低速挡、`100` 切换满速挡；不提供连续 PWM 或 RPM 调速。
...
只修改 Android；没有改写或烧录 STM32、ESP 固件。V1.0.0 已确认双电机（含 N20 唧筒）、±15° 及网页限角配套固件实装；本 APK 不能替其它客户端收窄固件/网页的行程。
