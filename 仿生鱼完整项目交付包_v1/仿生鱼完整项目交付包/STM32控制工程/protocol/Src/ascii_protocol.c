...
enum {
    ...
    FIELD_SERVO     = (1U << 4),
    FIELD_M2        = (1U << 5),
    /* m2 是**可选**字段：不在 REQUIRED 里，老客户端不发它也照样被接受。 */
...
#define RETURN_PAYLOAD_ERROR(error_code) do { \
    ...
    while (token != 0) {
        ...
        *equals = '\0';
        ...
        if (TextEquals(token, "seq")) {
        ...
        } else if (TextEquals(token, "m2")) {
            if (!MarkSeen(&seen, FIELD_M2)) {
                RETURN_PAYLOAD_ERROR(PROTO_ERR_DUP_FIELD);
            }
            if (TextEquals(equals, "F")) {
            ...
            if (!ParseSigned(equals, CFG_SERVO_SAFE_MIN_DEG, CFG_SERVO_SAFE_MAX_DEG, &value)) {
                RETURN_PAYLOAD_ERROR(PROTO_ERR_RANGE);
            }
...
/* BfMove -> STA 单字母。非法枚举值一律上报 S：状态帧宁可说"停"，
 * 也不能把未知值谎报成正在前进/后退。 */
...
size_t Protocol_EncodeStatus(char *out, size_t capacity, const BfSystemSnapshot *snapshot)
{
    ...
    written = snprintf(out, capacity,
                       ...
                       "roll=%s,pitch=%s,yaw=%s,m1=%c,m2=%c,err=%lu>\n",
