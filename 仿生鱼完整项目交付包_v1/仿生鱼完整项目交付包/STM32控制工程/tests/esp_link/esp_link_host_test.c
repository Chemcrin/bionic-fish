...
static void OnCommand(void)
{
    ...
    else if (strcmp(command, "AT+CIPSTA_CUR?") == 0)
        Input("+CIPSTA_CUR:ip:\"192.168.3.40\"\r\n+CIPSTA_CUR:gateway:\"192.168.3.1\"\r\n+CIPSTA_CUR:netmask:\"255.255.255.0\"\r\nOK\r\n");
    else if (strcmp(command, "AT+CIPSTA?") == 0)
        Input("+CIPSTA:ip:\"192.168.3.40\"\r\n+CIPSTA:gateway:\"192.168.3.1\"\r\n+CIPSTA:netmask:\"255.255.255.0\"\r\nOK\r\n");
...
/* HTTP 响应比事件槽(200B)大得多，而且**不能**一次 CIPSEND 投递 —— 串口 TX 环只有
 * 512 字节，一次性声明超过它的长度会永远入队失败并卡死事务（实机踩过）。
 * 这里验证：超长 raw 响应会被切成多段全部发出，且字节总数与原始长度一致。 */
static void TestRawChunkedResponse(void)
{
    static char big[900];
    unsigned i, before, total = 0U, chunks = 0U;

    Boot(false); Connect(0U);
    memset(big, 'x', sizeof(big) - 1U);
    assert(EspLink_SendRaw(big, sizeof(big) - 1U));
    before = payload_count;
    Run(6000U);
    for (i = before; i < payload_count; i++) {
        total += payload_lengths[i];
        chunks++;
    }
