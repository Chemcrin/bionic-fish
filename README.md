...
> 一只可在水下自主游动的仿生潜航器 —— 由**一人主导、AI Agent 联合开发**，完成从机械结构设计、电控硬件、嵌入式固件到 Android 上位机的全栈工程实现。外形参考「追鲨一号」构型，结构自主设计。面向课程设计、教学演示与水下任务原型验证。
...
> 🤖 **本项目由 AI Agent 联合开发。** 硬件方案推演、固件与协议实现、上位机、以及本文档本身，都在一个「AI Agent 可闭环」的流程中完成 —— 项目为此专门设计了 **ST-Link SWD 烧录 + 板载 CH340 串口日志**的双通道调试环境（见 [3.5.2](#352-板载-ch340-的实战价值双接-ai-agent-联合调试)），让工具能直接跑通「改代码 → 烧录 → 读日志 → 再改」的迭代闭环。
>
...
**这是一个人主导完成的全栈工程** —— 从外形与机械结构设计、电控硬件选型与拓展板搭建、嵌入式固件开发，到 Android 上位机与控制协议定义，全部由作者自主设计、自主实现（开发全程借助 AI Agent 协作，见文首说明），未采用现成开源飞控/机器人框架套用。
...
| 特点 | 说明 |
| --- | --- |
| **全栈单人造** | 机械结构、电路、固件、协议、上位机 App、网页控制台六层全部由一人打通（全流程与 AI Agent 协作，见文首说明）。跨层改动需要同时理解机械约束、时序约束与协议兼容性，没有分工兜底。 |
...
感谢在开发全程参与协作的 **AI Agent 助理工具** —— 从方案推演、样板代码、协议解析到日志排错与本文档的词句整理，本项目的「一人全栈」之所以可行，离不开这条人机协作链路。
