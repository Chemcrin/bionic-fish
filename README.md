...
> ⚠️ 本项目为教学原型，**尚处于台架验证阶段，未完成整机下水验收**。软件测试通过不等于整机可用，实测边界见[第十章](#十验收状态与已知边界)。
...
**设计取向是「用成熟模组换开发时间」**：ESP-01S 用 AT 固件而非自绘透明桥，IMU 用模块内部解算而非自写滤波，舵机用成品防水件而非自制密封 —— 每一处都选择了「贵一点但少一个失败点」的方案。
...
├── 电子信息方案/                 原理图、引脚分配、电源方案
└── 实物图片/                     结构件照片
```

> 📌 完整建模工作区（`建模/`，含外壳辅助脚本、历史版本、第三方素材）与讲演素材**不随仓库发布**，已在 `.gitignore` 中忽略，仅保留本地副本。
...
2. **串口观察**：通过 USART3 查看 `ESP server=... tcp=... reset=... error=...`。`server=1` 表示服务初始化完成，`tcp=1` 表示有控制客户端。
3. **连接**：手机加入 `BionicFish-AP`（密码 `12345678`），保持该无互联网 Wi-Fi，在 Android 设备页点击「AP 直连」。
4. **握手**：应用发送完整合法的 STOP CMD，收到同序号 ACK 与 `link=1` 的 STA 后才显示已连接。**加入 Wi-Fi 或连上 TCP 本身不等于握手完成。**
5. **台架验证**：先验证 STOP 的 ACK、STA、舵机与姿态，再点击前进做低速连续转动测试。确认台架启停、失联、ESP 复位、供电恢复后再考虑下水。
...
| 项目 | 结果 |
| --- | --- |
| 固件主机回归测试 | **16/16 通过**（含编译期门禁） |
| Arm 交叉构建 | 通过，BIN **34,272 字节** |
| 固件资源占用 | Flash **52.29%**、RAM **79.92%** |
| Android 单元测试 | **107/107 通过** |
| Android 仪器测试 | **28/28 通过**（API 35 模拟器） |
| Android Lint | **0 errors / 4 warnings**（均为依赖更新提示） |
| APK 签名与对齐 | apksigner 通过、zipalign 通过 |

固件产物 SHA-256：`2559592197244234f67009fd26682a8785665dbe6f5fa6f692f3113653029e4a`
APK SHA-256：`F61B6DF96F7C374D8CF85C5D9A15A7E813EBD27C224A1C8FDEBCC769DB759479`

### 10.2 尚未完成的验证

> ⚠️ **软件测试通过 ≠ 整机可用。** 以下为本轮明确**未验证**的项，复刻者需自行实测：

- **实板整机联调**：STM32 / ESP / Android 三方完整 TCP 链路未在实物上跑通；
- **机械与电气实测**：两路电机的转向、电流、温升；**2 s 慢周期 + 95% 占空比下 TB6612 能否稳定带载**；堵转保护（TB6612 **无过流保护规格**，这是硬缺口）；
- **水下试验**：整机从未下水，防水、配重、姿态稳定性均未验证；
- **传感器核对**：JY901S 寄存器映射、OLED 热恢复；
- **手机兼容性**：三星 S24 安装、AP 无互联网与移动数据共存行为未实测。

### 10.3 已知待改进项

| 项 | 说明 | 建议方向 |
| --- | --- | --- |
| App 连上就断 | `AT+CIPSTO=3` 使 ESP 3 秒空闲即断开，而 App 在无命令时只 delay 不发帧 | 放宽 `CIPSTO`（如 30 s），或让 App 发心跳 |
| 单客户端限制 | 固件只接受一个控制客户端，安卓与网页无法同时连接 | 按需扩展控制客户端槽位 |
| 按键功能缺口 | 板载 K1/K2/K3 只能调试 M1 与舵机，**没有任何按键能控制 M2**（M2 唧筒已可由协议 `m2=` 与上位机正常调用，此处仅指实体按键） | 补充 M2 按键映射 |
| IMU 上拉 | 当前默认走内部 40 kΩ 上拉台架兼容模式，抗 EMI 差 | 焊上 4.7 kΩ 外部上拉后改回标准开漏 |

### 10.4 安全须知

> 🔴 **下水前必须确认**：
> 1. `CFG_DEBUG_BUTTONS_ENABLED` 已设回 **0** —— 该功能有意放开了「无链路即无运动」的看门狗保证，K1 启动的转动在完全没有上位机时会**一直持续**；
> 2. 两路电机与舵机已装好机械保护，可立即断电；
> 3. 已完成台架启停、失联、ESP 复位与供电恢复测试；
> 4. 10% / 60% / 95% 均为**开环电压设定，不是限流或过流保护**。
