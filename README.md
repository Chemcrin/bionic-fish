...
> 一只可在水下游动的仿生潜航器 —— 由**项目组主导、组员协作、AI Agent 辅助**，完成从机械结构设计、电控硬件、嵌入式固件到 Android 上位机的全栈工程实现。外形参考「追鲨一号」构型，结构自主设计。**整机已完成实际下水运行（泳池 / 静水实测）**，面向课程设计、教学演示与水下任务原型验证。

<p align="center">
  <img src="docs/images/field-pre-dive.jpg" width="100%" alt="整机下水前实拍">
</p>
<p align="center"><em>整机实拍 —— 头 / 体 / 尾三段分件，尾部曲柄摇杆传动与尾鳍外露可见</em></p>

<p align="center">
  <img src="docs/images/demo.gif" width="74%" alt="整机在泳池中游动的实拍演示">
</p>
<p align="center"><em>实机演示 —— 整机在泳池中自主游动，尾鳍摆动推进；完整 24 秒视频见 <a href="docs/media/demo.mp4">docs/media/demo.mp4</a></em></p>

<p align="center">
  <img src="docs/images/machine-overview.jpg" width="47%" alt="整机俯视实拍">
  <img src="docs/images/pcb-board.jpg" width="47%" alt="自研拓展板实装照片">
</p>
<p align="center"><em>左：整机俯视（白色头部 / 蓝色鱼体 / 白色尾段）；右：自研拓展板实装 —— TB6612、ESP-01S、CH340 + Type-C、OLED、XT60 与 LM2596S 全部可拔换</em></p>

<p align="center">
  <img src="docs/images/shell-iso.png" width="47%" alt="外壳外观设计渲染">
  <img src="docs/images/prototype-body.jpg" width="47%" alt="3D 打印实物">
</p>
<p align="center"><em>外观设计渲染与 3D 打印实物</em></p>

**开源地址：** [Chemcrin/bionic-fish](https://github.com/Chemcrin/bionic-fish)

> 🤖 **本项目由项目组与 AI Agent 协作开发。** 硬件方案推演、固件与协议实现、上位机、以及本文档本身，都在一个「AI Agent 可闭环」的流程中完成 —— 项目为此专门设计了 **ST-Link SWD 烧录 + 板载 CH340 串口日志**的双通道调试环境（见 [3.5.2](#352-板载-ch340-的实战价值双接-ai-agent-联合调试)），让工具能直接跑通「改代码 → 烧录 → 读日志 → 再改」的迭代闭环。
>
> **如果你也打算复刻，我们鼓励你同样合理地使用 AI 助理工具辅助复刻** —— 让工具去承担资料检索、样板代码、协议解析与日志排错，把人的精力留给机械取舍、电气判断与实机验证。
>
> ⚠️ 但请守住底线：AI 产出的内容**必须经过你自己的复核与实测**。尤其是电源极性、接线顺序、保险丝规格与下水安全相关的部分，**不要未经确认就直接照抄执行**。
...
- [八、仓库结构](#八仓库结构)
- [九、快速开始](#九快速开始)
- [📖 复刻指南（硬件制造 / 通信调试 / 电控编写）](docs/复刻指南/README.md)
...
**这是由项目组主导、组员协作完成的全栈工程** —— 从外形与机械结构设计、电控硬件选型与拓展板搭建、嵌入式固件开发，到 Android 上位机与控制协议定义，均由项目组自行设计与实现（开发全程借助 AI Agent 辅助，见文首说明），未采用现成开源飞控/机器人框架套用。
...
| 特点 | 说明 |
| --- | --- |
| **全栈自研** | 机械结构、电路、固件、协议、上位机 App、网页控制台六层全部由项目组自行打通（全流程有 AI Agent 辅助，见文首说明；制作与验证环节由组员协作承担）。跨层改动需要同时理解机械约束、时序约束与协议兼容性。 |
...
| **已实际下水运行** | 整机装配完成后**真实入水运行**，在泳池 / 静水环境验证密封、配重、姿态与基本机动（前进、转向、安全停机），不是只停留在台架或仿真阶段。 |

**首版定位**：无线遥控 + 半自动控制原型，核心目标是验证「能游、能控、能安全停机」—— 该目标已通过整机下水实测确认；视觉自主与深度闭环留待第二阶段增强。

> ⚠️ 本项目为教学原型，**已在泳池 / 静水环境完成整机下水运行实测**。但**长时间深水作业、复杂水域与自主任务能力尚未验证**；软件测试通过也不等于任意环境下都可用。每次下水前仍请确认已装好机械保护、可立即断电，并已完成台架启停、失联与供电恢复测试。

> 📌 **关于文中「需实测确认」的表述**：软件层证据（host 回归、Android 单元/仪器测试、实板烧录与 USART3 诊断）与整机下水是**两套独立证据**。文中标注为「待实板」「需实测」的条目，指**尚未做过逐项定量标定**的项目（如堵转电流、TB6612 温升、动密封长期衰减），不代表整机未下水。
...
<p align="center">
  <img src="docs/images/pcb-3d.jpg" width="86%" alt="拓展板 PCB 3D 设计视图">
</p>
<p align="center"><em>拓展板 PCB 3D 设计视图（立创EDA）—— 丝印上标出了每个模组的插接位置与信号名，装配时对着丝印插即可</em></p>
...
> **设计归属说明**：本章结构设计由**项目组自主完成**（组员协作参与建模与实物验证）。外观上**参考了「追鲨一号」潜航器这一公开外形作为风格参考** —— 即借鉴其整体构型语汇（盾形前脸、三角侧翼、圆筒尾部护罩、顶部把手等）作为设计起点，**并非照搬或复刻其模型**；所有尺寸、型线、分件与内部结构均为项目组自主设计与权衡的结果。
>
...
**设计主线**：项目组先在 SolidWorks 中确立分件方案与关键结构，再借助 Python 脚本把定义好的型线快速展开成完整网格，用于预览渲染与打印试制；需要修改外形时改脚本参数即可快速试算，最终结构仍以 CAD 模型为准。
...
<p align="center">
  <img src="docs/images/body-cad.jpg" width="72%" alt="鱼体 CAD 模型正视图">
</p>
<p align="center"><em>鱼体在 CAD 中的正视图 —— 型线剖面、分件接缝与侧面菱形开口／舷鳍安装面</em></p>
...
<p align="center">
  <img src="docs/images/tail-cad.jpg" width="76%" alt="尾部曲柄摇杆传动装配（CAD 装配体）">
</p>
<p align="center"><em>尾部传动装配（CAD）—— 电机座 → 曲柄 → 摇杆 → 尾鳍，支撑件即上表中的 V6F / V4F</em></p>
...
- 唧筒活塞在注射器筒体内的**动密封**是全机防水的关键风险点：已随整机下水运行验证可用，但长期往复后的密封衰减、以及深水压力下的表现尚未做过定量测试。
...
├── docs/
│   ├── images/                  文档配图、实拍照片与演示动图（demo.gif）
│   ├── media/                   演示视频（H.264 MP4，浏览器可在线播放）
│   └── 复刻指南/                  ★ 硬件制造 / 通信调试 / 电控编写
├── 仿生鱼完整项目交付包_v1/      ★ 完整交付包
│   └── 仿生鱼完整项目交付包/
...
| 我想… | 去哪里 |
| --- | --- |
| **从零复刻一台** | [**`docs/复刻指南/`**](docs/复刻指南/README.md) ← 硬件制造 · 通信调试 · 电控编写 |
| 了解当前固件与硬件边界 | [`STM32控制工程/README.md`](仿生鱼完整项目交付包_v1/仿生鱼完整项目交付包/STM32控制工程/README.md) |
| 编译固件 | [`scripts/BUILDING.md`](仿生鱼完整项目交付包_v1/仿生鱼完整项目交付包/STM32控制工程/scripts/BUILDING.md) |
| 看通信协议细节 | [`通信协议.md`](仿生鱼完整项目交付包_v1/仿生鱼完整项目交付包/STM32控制工程/通信协议.md) |
| 查引脚与驱动 | [`硬件引脚与驱动说明.md`](仿生鱼完整项目交付包_v1/仿生鱼完整项目交付包/STM32控制工程/硬件引脚与驱动说明.md) |
| 查电源架构与电压 | [`电源方案与电压汇总.md`](电气设计说明/电源方案与电压汇总.md) |
| 查拓展板接线与调试通道 | [`拓展板接口与调试通道.md`](电气设计说明/拓展板接口与调试通道.md) |
| 编译/安装 Android 应用 | [`Android工程/README.md`](仿生鱼完整项目交付包_v1/仿生鱼完整项目交付包/仿生鱼安卓_v0.5.0_交付包/Android工程/README.md) |
| 取尾段结构件 | [`鱼尾装配/README.md`](鱼尾装配/README.md) |
| 看验收边界 | [`验收检查清单.md`](仿生鱼完整项目交付包_v1/仿生鱼完整项目交付包/STM32控制工程/验收检查清单.md) |
...
7. **台架验证**：先验证 STOP 的 ACK、STA、舵机与姿态，再点击前进做低速连续转动测试。确认台架启停、失联、ESP 复位与供电恢复均正常后，才进入下水环节。
8. **下水运行**：篮筐 / 系绳保护下入水，先在浅水确认密封无渗漏、配重与姿态稳定，再做前进、转向与安全停机测试；异常时立即断电回收。本机已按此流程完成泳池 / 静水实测。
...
| **实机下水** | 9 月 | 整机装配完成并**实际入水运行**，在泳池 / 静水环境完成密封、配重、姿态与基本机动的下水实测 |
...
感谢在项目制作、联调与实机下水验证中承担工作的**组员同学** —— 机械件的加工装配、电气接线与下水现场的回收复盘，都是集体完成的。
感谢在开发全程参与协作的 **AI Agent 助理工具** —— 从方案推演、样板代码、协议解析到日志排错与本文档的词句整理，本项目能在课程周期内打通全栈，离不开这条人机协作链路。

> 💡 如果你正在复刻本项目，欢迎同样把 AI 助理工具纳入你的工作流；但请记住：**工具负责快，人负责对**。所有涉及电气安全与实机行为的结论，都必须由你在真实硬件上验证过才能采信。
...
随着**AI Agent**和各家模型的迅速进步，他们在各领域的出场率越来越高，在项目上的职能占比也日渐扩大。
这使得个人全栈能力及效率大幅提升、项目单人可实现的同时，也影响了初学者针对基础项目能力开发的忽视。
Agent是减少重复劳动的重要工具，但他目前也还是工具，暂时还不能完全替代开发者思考和布局。
希望各位都能找到属于自己的核心能力，在复刻、调试、二次创造的过程里不断沉淀，不断前进。
